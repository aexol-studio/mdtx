import { useRepositoryState } from '@/src/containers';
...
export const CommitModal: React.FC<ICommitModal> = ({
    ...
    const { selectedBranch } = useRepositoryState();
    return (
        <div className="w-full h-full justify-center items-center">
            {submittingCommit ? (
                ...
                <form
                    ...
                    <p className="select-none text-white text-[1.4rem]">Commit head message</p>
                    <Controller
                        defaultValue=""
                        ...
                        render={({ field: { onChange, value } }) => (
                            <Input value={value} onChange={onChange} placeholder="Commit head message" />
                        )}
                    />

                    <p className="mt-[0.8rem] select-none text-white text-[1.4rem]">Commit body message</p>
                    <Controller
                        defaultValue=""
                        control={controlCommit}
                        ...
                        rules={{ required: true }}
                        render={({ field: { onChange, value } }) => (
                            <Input value={value} onChange={onChange} placeholder="Commit body message" />
                        )}
                    />
